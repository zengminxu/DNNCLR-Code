        ...
        for [data1, data2, data3], label in loader:
            ...
            if epoch <= self.arg.mining_epoch:  
                ...
                if hasattr(self.model, 'module'):
                    self.model.module.update_ptr(output2.size(0))
                else:
                    self.model.update_ptr(output2.size(0))
                loss1 = self.loss(output1, target1)
                loss2 = -torch.mean(torch.sum(torch.log(output2) * target2, dim=1))  # DDM loss  Ld1
                loss3 = -torch.mean(torch.sum(torch.log(output3) * target2, dim=1))  # DDM loss  Ld2
                loss = loss1  + (loss2 + loss3) / 2.  #  loss1 + + 0.1 * dtw_loss # L1 = Linfo + LD ; LD = 1/2(Ld1+Ld2)
            else:  
                ...
                loss = loss1  + (loss2 + loss3) / 2. # loss1 + L2 = LN + LD
            ...
            # self.iter_info['nn_loss'] = nn_loss.data.item()
            # nn_loss_value.append(self.iter_info['nn_loss'])
