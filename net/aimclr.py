...
class AimCLR(nn.Module):
    def __init__(self, base_encoder=None, pretrain=True, feature_dim=128, queue_size=32768,
        ...
        self.memory_bank = NNMemoryBankModule(size=supsize)
        # self.criterion = NTXentLoss(memory_bank_size=32768)
    ...
    def forward(self, im_q_extreme, im_q, im_k=None, nnm=False, topk=1):
        ...
        q = F.normalize(q, dim=1)  # [32.128]
        q_extreme = F.normalize(q_extreme, dim=1)
        q_extreme_drop = F.normalize(q_extreme_drop, dim=1)
        
        # q = self.memory_bank(q, update=True)

        # Compute key features
        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder()  # update the key encoder
            k = self.encoder_k(im_k)  # keys: NxC
            k = F.normalize(k, dim=1)  # [32.128]
            k = self.memory_bank(k, update=True)
        ...
        labels = torch.zeros(logits.shape[0], dtype=torch.long).cuda()  # infoNCE的labels
        ...
        # logits_clu, lable_clu = sim(q, k,self.queue.clone().detach(), self.T)
        # z0 = self.memory_bank(q, update=False)
        # z1 = self.memory_bank(k, update=True)
        # nn_loss_1 , logits_nn_1 = self.criterion(z0, z1)
        # nn_loss_2 , logits_nn_2 = self.criterion(z1, z0)
        # nn_loss = 0.5 * ( nn_loss_1 + nn_loss_2)
